...
"""
Web Control Node v4 – Strict mode switching, minimal delay.

Architecture:
  Joystick  → /joy → joystick_control.py → /cmd_vel/manual
  Web D-pad → /cmd_vel/web
  Auto      → /cmd_vel/auto
  This node  → /web_control_mode (String) → cmd_vel_mux
            → /auto_enable (Bool)         → cmd_vel_mux

Modes: WEB | JOY | AUTO  – only one active at a time.
"""
    ...
    <style>
        *,*::before,*::after{box-sizing:border-box;-webkit-tap-highlight-color:transparent;margin:0;padding:0}
        ...
        header{
            ...
            justify-content:space-between;padding:2px 0 6px
        }
        .logo{font-size:1rem;font-weight:500;letter-spacing:.05em;color:#60a5fa}
        .logo span{color:#818cf8}
        .pills{display:flex;gap:6px}
        .pill{
            font-size:.6rem;padding:2px 7px;border-radius:20px;font-weight:500;
            letter-spacing:.05em;background:var(--sf2);color:var(--mut);
            border:1px solid var(--br);transition:.25s
        }
        ...
        .ptitle{
            font-size:.65rem;font-weight:500;color:var(--mut);letter-spacing:.1em;
        ...
        .tab{
            background:var(--sf2);color:var(--mut);border:1px solid var(--br);
            border-radius:8px;padding:9px 0 8px;font-size:.72rem;
            font-family:var(--font);cursor:pointer;display:flex;
            flex-direction:column;align-items:center;gap:4px;font-weight:500;
        ...
        .sbar{
            ...
            padding:7px 10px;background:var(--sf2);border-radius:7px;
            font-size:.72rem;border:1px solid var(--br)
        }
        .sbar .led{width:7px;height:7px;border-radius:50%;flex-shrink:0;transition:.3s}
        .led-g{background:var(--grn)}
        .led-y{background:var(--amb)}
        .led-r{background:var(--red)}
        .led-b{background:var(--acc)}
        ...
        .sl-row .sv{
            font-size:.72rem;font-weight:500;color:var(--acc);
        ...
        .db{
            background:var(--sf2);color:var(--txt);border:1px solid var(--br);
            border-radius:9px;padding:15px 0;font-size:1.25rem;cursor:pointer;
        ...
        .sonar-cell{
            background:var(--sf2);border-radius:7px;padding:7px 6px;
            text-align:center;border:1px solid var(--br)
        }
        ...
        .joy-box{
            background:var(--sf2);border-radius:8px;padding:10px;border:1px solid var(--br)
        }
        .joy-box .jlabel{font-size:.65rem;color:var(--mut);margin-bottom:4px}
        .joy-bar-wrap{
            height:6px;background:var(--br);border-radius:3px;
            overflow:hidden;margin-top:6px
        }
        .joy-bar{
            height:100%;background:var(--pur);border-radius:3px;
            transition:width .08s;width:50%
        }
        ...
        .param-row .pv{
            font-size:.72rem;font-weight:500;color:var(--pur);
            min-width:38px;text-align:right
        }
        .apply-btn{
            margin-top:8px;width:100%;padding:8px;background:#1a1040;
            color:#c4b5fd;border:1px solid #3730a3;border-radius:7px;
            font-family:var(--font);font-size:.72rem;cursor:pointer;
        ...
        .s-cell{
            background:var(--sf2);border-radius:7px;padding:8px;
            text-align:center;border:1px solid var(--br)
        }
...
<body>
    ...
    <script>
        ...
        function bindSlider(id, outId, fixed=2) {
            ...
            el.oninput = () => {
                ov.textContent = parseFloat(el.value).toFixed(fixed);
                if (holdActive && mode === 'WEB') sendDir(currentDir);
            };
        }
        bindSlider('sLin','vLin');
        bindSlider('sAng','vAng');
        bindSlider('pLin','pvLin');
        bindSlider('pAng','pvAng');
        ...
        // ═══════════════════════════════════════════════════════════════
        // D-pad (Web mode) – no setInterval, server keep-alive
        // ═══════════════════════════════════════════════════════════════
        function sendDir(d) {
            if (mode !== 'WEB') return;
            ...
            const map = {
                f:{linear_x:lin,angular_z:0},
                b:{linear_x:-lin,angular_z:0},
                l:{linear_x:0,angular_z:ang},
                r:{linear_x:0,angular_z:-ang}
            };
        ...
        document.querySelectorAll('.db').forEach(btn => {
            ...
            btn.addEventListener('mousedown', () => d === 's' ? (releaseHold(), api('/api/stop')) : pressDir(d));
            btn.addEventListener('mouseup', releaseHold);
            btn.addEventListener('mouseleave', releaseHold);
            btn.addEventListener('touchstart', e => {
                e.preventDefault();
                d === 's' ? (releaseHold(), api('/api/stop')) : pressDir(d);
            }, {passive:false});
            btn.addEventListener('touchend', releaseHold);
            btn.addEventListener('touchcancel', releaseHold);
        });
        ...
        document.addEventListener('keydown', e => {
            ...
            if (map[e.key]) {
                e.preventDefault();
                map[e.key] === 's' ? (releaseHold(), api('/api/stop')) : pressDir(map[e.key]);
            }
        });
        ...
        function drawRadar() {
            ...
            [0.25, 0.5, 1.0, 1.5].forEach(r => {
                const px = (r / MAX_M) * 80;
                rc.beginPath();
                rc.arc(RC, RC, px, 0, 2*Math.PI);
                rc.strokeStyle = '#1e2430';
                rc.lineWidth = 1;
                rc.stroke();
                rc.fillStyle = '#2d3748';
                rc.font = '9px DM Mono,monospace';
                rc.fillText(r+'m', RC + px + 2, RC);
            });

            rc.strokeStyle = '#1a2030';
            rc.lineWidth = 1;
            ...
            rc.fillStyle = '#1e2d40';
            rc.beginPath();
            rc.roundRect(RC-14, RC-18, 28, 36, 5);
            rc.fill();
            ...
            sensors.forEach(({key, start, end, id}) => {
                ...
                rc.arc(RC, RC, px, s, e);
                rc.closePath();
                rc.fillStyle = sonarColor(m);
                rc.fill();
                rc.strokeStyle = '#2d3748';
                rc.lineWidth = 1;
                rc.stroke();
        ...
        setInterval(async () => {
            try {
                ...
                if (d.joy_axes && d.joy_axes.length >= 2) {
                    const ax0 = -d.joy_axes[0];
                    const ax1 = -d.joy_axes[1];
...
class WebControlNode(Node):
    def __init__(self):
        ...
        # ── Publishers ───────────────────────────────────────────────────────
        self.pub_cmd_web    = self.create_publisher(Twist,  '/cmd_vel/web',     10)
        self.pub_mode_cmd   = self.create_publisher(String, '/web_control_mode', 10)
    ...
    # ── Keep-alive ───────────────────────────────────────────────────────────
    def _keepalive(self):
        if self._web_active and self.current_mode == 'WEB':
            self.pub_cmd_web.publish(self._last_twist)
    ...
    def stop(self):
        self._last_twist    = Twist()
        self._web_active    = False
        self.pub_cmd_web.publish(Twist())

    def set_mode(self, mode: str):
        """Called from Flask when user clicks mode tab."""
        if mode not in ('WEB', 'JOY', 'AUTO'):
            return
        old = self.current_mode
        self.current_mode = mode

        # Publish desired mode for cmd_vel_mux
        m = String()
        m.data = mode
        self.pub_mode_cmd.publish(m)

        # Stop web commands if not WEB
        if mode != 'WEB':
            self.stop()

        # Manage auto process
        if mode == 'AUTO' and old != 'AUTO':
            self._start_auto_process()
        elif mode != 'AUTO' and old == 'AUTO':
            self._stop_auto_process()

    def _start_auto_process(self):
        self.set_auto_enable(True)
        global auto_proc
        with _lock:
            if auto_proc is None or auto_proc.poll() is not None:
    ...
    def _stop_auto_process(self):
        self.set_auto_enable(False)
        global auto_proc
        with _lock:
            if auto_proc and auto_proc.poll() is None:
                auto_proc.send_signal(signal.SIGINT)
                try:
                    auto_proc.wait(timeout=3)
                except:
                    auto_proc.kill()
                    auto_proc.wait(timeout=2)
            auto_proc = None
...
def mv():
    ...
    if node:
        node.move(d.get('linear_x', 0), d.get('angular_z', 0))
...
def api_stop():
    if node:
        node.set_mode('JOY')  # fallback to JOY after stopping auto
